...
class SearchGenreCountryViewModel(mode: Int, kinopoiskRepository: KinopoiskRepository): ViewModel() {
    private val data = if (mode == 1)
        kinopoiskRepository.listGenres!!.map { GenreOrCountry(it.id, it.genre) }.sortedBy { it.text }
    else
        kinopoiskRepository.listCountries!!.map { GenreOrCountry(it.id, it.country) }.sortedBy { it.text }
